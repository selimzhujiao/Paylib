...
public class PayForMoneyActivity extends  Activity {
			...
			public void handleMessage(Message msg) {
				...
				case SDK_PAY_FLAG: {
					...
					if (TextUtils.equals(resultStatus, "9000")) {
						Toast.makeText(PayForMoneyActivity.this, "支付成功", Toast.LENGTH_SHORT).show();
					} else {
						// 判断resultStatus 为非"9000"则代表可能支付失败
						// "8000"代表支付结果因为支付渠道原因或者系统原因还在等待支付结果确认，最终交易是否成功以服务端异步通知为准（小概率状态）
						if (TextUtils.equals(resultStatus, "8000")) {
							Toast.makeText(PayForMoneyActivity.this, "支付结果确认中", Toast.LENGTH_SHORT).show();

						} else {
							// 其他值就可以判断为支付失败，包括用户主动取消支付，或者系统返回的错误
							Toast.makeText(PayForMoneyActivity.this, "支付失败", Toast.LENGTH_SHORT).show();
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.pay_for_money_layout);
	...
	public void pay(View v) {
		...
		Runnable payRunnable = new Runnable() {
			...
			public void run() {
				// 构造PayTask 对象
				PayTask alipay = new PayTask(PayForMoneyActivity.this);
				// 调用支付接口，获取支付结果
